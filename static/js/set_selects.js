...
let selectComunaButton;

document.addEventListener('DOMContentLoaded', function() {

    console.log("hola");
    
    selectRegionButton = document.getElementById('region');
    selectComunaButton = document.getElementById('comuna');
    ...
    regiones = regcom.regiones.map(region => region.nombre);

    set_to_group(regiones, selectRegionButton);
});

function getComunasByRegion(regionName) {
    const region = regcom.regiones.find(region => region.nombre === regionName);
    if (region) {
        return region.comunas.map(comuna => comuna.nombre);
    } else {
        return [];  // Return empty array if region not found
    }
}

function set_to_group (list, group) {
...
function setComunasButton () {
    selectComunaButton.innerHTML = "";
    set_to_group(getComunasByRegion(document.getElementById('region').value), selectComunaButton);
}
